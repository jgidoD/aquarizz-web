...
import { serverTimestamp, doc, getDoc, collection, addDoc, getDocs, query, orderBy, limit } from "firebase/firestore";
      ...
      <div>
        {
          comment && comment.map((doc) => (
            ...
            <UserProfileButton userProfileID={doc.authorID} userProfileName={doc.name}/>
        ...
        <form onSubmit={handleSubmitComment} className="commentForm">
            <textarea className="commentInput" type="text" name="commentContent" rows={2} cols={50} onChange={handleChange} ref={txt}/>

            <div className="postCommentBtn">
                <input className="postContentBtn" type="submit" value="Comment" onClick={fetchComments} />
            </div>
        </form>
    </div>)
}